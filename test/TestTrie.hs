...
arbitraryProofTrie = do
  ...
  vals <- map (Leaf []) <$> infiniteListOf arbitrary
  let hexMap = zip (sort $ nub $ keys) vals
  let trie = hexMapToTrie hexMap
  (nibs, bs) <- oneof $ pure <$> hexMap
  pure $ (trie, nibs, bs)

prop_proof :: (Trie, Nibbles, Trie) -> Bool
prop_proof (trie,nibs,leaf) =
  trieRoot trie == trieRoot (trieProof nibs leaf $ trieProof nibs (Leaf [] "") trie)
...
trieTests = testGroup "trie"
  ...
  , testCase "regression 1" $
      let trie = Branch16 [N,N,N,Branch16 [N,Prefix [15,7] $ Leaf [] " ",N,N,N,Branch16 [N,N,N,N,N,N,Branch16 [N,N,N,N,N,N,N,N,N,N,N,N,N,N,N,N] "",N,N,N,N,N,N,N,N,N] "",N,N,N,N,N,N,N,N,N,N] " ",N,N,N,N,N,N,N,N,N,N,N,N] " "
       in prop_proof (trie, [3,1,15,7], Leaf [] " ") @?= True
  ...
  , testCase "branch16" $
      let trie = hexMapToTrie [([], Leaf [] "a"), ([1], Leaf [] "b")]
  ...
  , testCase "orderedTrie1" $ do
      ...
      let trie = trieProof [] (Leaf [] "") $ orderedTrie ["\2", BS.replicate 33 1]
  ...
  ,  testCase "embedTrie" $ do
       let l = BS.replicate 33 45
           a = Leaf [] l
           b = trieMapToTrie [("0", hashNode a), ("1", Leaf [] "")]
           c = trieMapToTrie [("0", a), ("1", Leaf [] "")]

       hashNode b @?= hashNode c
  ]
